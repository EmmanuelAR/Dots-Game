...
SeisPuntos::SeisPuntos():CampoAbs()
{
	this->f = 5;
	this->c = 3;
	m = new Elemento * *[f];
...
bool SeisPuntos::insertar(int x, int y, int n)
{
	Elemento* ele = NULL;
...
bool SeisPuntos::insertarNumero(int n)
{
	Elemento* nu = NULL;
	for (int i = 0; i < f; i++) {
		for (int j = 0; j < c; j++) {
...
int SeisPuntos::getTamFila()
{
	return f;
}

int SeisPuntos::getTamColumna()
{
	return c;
}
